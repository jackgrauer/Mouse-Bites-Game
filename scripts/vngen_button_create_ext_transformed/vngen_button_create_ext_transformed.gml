/// @function	vngen_button_create_ext_transformed(id, text, sprite, spr_hover, spr_select, xorig, yorig, x, y, z, tx, ty, scaling, font, color, hov_x, hov_y, hov_xscale, hov_yscale, hov_color, sel_x, sel_y, sel_xscale, sel_yscale, sel_color, sel_duration, snd_hover, snd_select, transition, duration, ease);
...
function vngen_button_create_ext_transformed() {
	...
	argument15 = sets the horizontal offset to display the button when hovered (real)
	argument16 = sets the vertical offset to display the button when hovered (real)
	argument17 = sets the horizontal scale multiplier to display the button when hovered (real)
	argument18 = sets the vertical scale multiplier to display the button when hovered (real)
	argument19 = sets the button text color to display when hovered (color)
	argument20 = sets the horizontal offset to display the button when selected (real)
	argument21 = sets the vertical offset to display the button when selected (real)
	argument22 = sets the horizontal scale multiplier to display the button when selected (real)
	argument23 = sets the vertical scale multiplier to display the button when selected (real)
	argument24 = sets the button text color to display when selected (color)
	argument25 = sets the length of hover/select animations, in seconds (real)
	argument26 = sets the sound to be played when hovered (sound) (optional, use 'none' for none)
	argument27 = sets the sound to be played when selected (sound) (optional, use 'none' for none)
	argument28 = sets the transition script to perform (script) (optional, use 'trans_none' for none)
	argument29 = sets the length of the transition, in seconds (real)
	argument30 = sets the transition easing override (see interp for available modes) (integer) (can also use true/false) (optional, use no argument for none)

	Example usage:
	   vngen_event() {
	      vngen_button_create("btn", "Click me!", spr_btn, spr_btn_hover, spr_btn_select, 0, 0, 256, 512, 0, 72, 16, 0, fnt_Arial, c_white, 32, 0, 1.25, 1.25, c_black, 0, 0, 1, 1, c_yellow, 0.1, snd_hover, snd_select, trans_fade, 3, true);
	   }
	*/
	...
	*/
	...
	switch (sys_action_init()) {
	   ...
	   case true: {
	      ...
	      ds_button[# prop._snd_hover, ds_target] = argument[26];						//Hover sound
	      ds_button[# prop._snd_select, ds_target] = argument[27];						//Select sound
	      ...
	      ds_button[# prop._anim_tmp_col1, ds_target] = argument[19];            //Hover text color
	      ds_button[# prop._anim_tmp_x, ds_target] = argument[7] + argument[15]; //Hover X
	      ds_button[# prop._anim_tmp_y, ds_target] = argument[8] + argument[16]; //Hover Y
	      ds_button[# prop._anim_tmp_xscale, ds_target] = argument[17];          //Hover X scale
	      ds_button[# prop._anim_tmp_yscale, ds_target] = argument[18];          //Hover Y scale
      
	      //Set select properties
	      ds_button[# prop._anim_col1, ds_target] = argument[24];                //Select text color
	      ds_button[# prop._anim_x, ds_target] = argument[7] + argument[20];     //Select X
	      ds_button[# prop._anim_y, ds_target] = argument[8] + argument[21];     //Select Y
	      ds_button[# prop._anim_xscale, ds_target] = argument[22];              //Select X scale
	      ds_button[# prop._anim_yscale, ds_target] = argument[23];              //Select Y scale
      
	      //Set timing properties
	      ds_button[# prop._anim_time, ds_target] = 0;                           //Animation time
	      ds_button[# prop._anim_dur, ds_target] = max(0.01, argument[25]);      //Animation duration
	      ds_button[# prop._anim_ease, ds_target] = argument[30];                //Animation ease
	      ...
	      sys_trans_init(ds_button, ds_target, argument[28], argument[29], false);
	...
	*/
	...
	if (ds_button[# prop._trans_time, ds_target] < ds_button[# prop._trans_dur, ds_target]) {
	   sys_trans_perform(ds_button, ds_target, argument[30]);

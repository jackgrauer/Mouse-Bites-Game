/// @function	vngen_set_shader_float(id, type, [name], uniform, value);
...
function vngen_set_shader_float() {

	/*
	Assigns a floating point value to be passed into a shader, if any is active
	on the given entity.
	...
	argument4 = the value to assign to the shader uniform (real/float)

	Example usage:
	   vngen_set_shader_float("bg", vngen_type_scene, "uniform0", 0.0);
	*/
	...
	ds_uniform = ds_data[# prop._sh_float_data, ds_target];

	//Create uniform data structure, if none exists
	if (!ds_exists(ds_uniform, ds_type_grid)) {
	   ds_data[# prop._sh_float_data, ds_target] = ds_grid_create(3, 0);
	   ds_uniform = ds_data[# prop._sh_float_data, ds_target];
	}
	...
	ds_uniform[# prop._val, ds_uni_target] = argument[3 + arg]; //Value

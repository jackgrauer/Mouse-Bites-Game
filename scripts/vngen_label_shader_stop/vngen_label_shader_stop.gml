/// @function	vngen_label_shader_stop(id, fade, [ease]);
/// @param		{real|string|macro}	id
/// @param		{real}				fade
/// @param		{integer|macro}		[ease]
/// @author		Lucas Chasteen <lucas.chasteen@xgasoft.com>
/// @copyright	XGASOFT 2020, All Rights Reserved
function vngen_label_shader_stop() {

	/*
	Stops a shader being applied to the specified label, if any.
	...
	switch (sys_action_init()) {
	   ...
	   case true: {
	      ...
	      if (argument[0] == all) {
	         ...
	         //Otherwise get single label to animate
	...
	*/
	...
	while (ds_target >= ds_yindex) { 
	   ...
	   if (ds_label[# prop._sh_time, ds_target] < argument[1]) {
	      if (!sys_action_skip()) {
	         ds_label[# prop._sh_time, ds_target] += time_frame;
	      } else {
	         //Otherwise skip transition if vngen_do_continue is run
	         ds_label[# prop._sh_time, ds_target] = argument[1];
	      }
      
	      //Mark this action as complete
	      if (ds_label[# prop._sh_time, ds_target] < 0) or (ds_label[# prop._sh_time, ds_target] >= argument[1]) { 
	         //Disable shader
	         ds_label[# prop._shader, ds_target] = -1;
	   ...
	   if (argument[1] > 0) {
	      ...
	      //Get transition time
	      var action_time = interp(0, 1, ds_label[# prop._sh_time, ds_target]/argument[1], action_ease);
   
	      //Perform shader fade
	      ds_label[# prop._sh_amt, ds_target] = lerp(1, 0, action_time); //Amount, or fade percentage
	   }

/// @function	vngen_set_prop(id, type, [name], prop, val);
...
function vngen_set_prop() {

	/*
	************************************************************************************
	WARNING: This script modifies internal VNgen behavior and has the potential to break
			 things. Use at your own risk!
	************************************************************************************

	Modifies the value of an explicit property of the specified entity.
	...
	argument3 = the property to modify value of (enumerator) 
	argument4 = the value to assign to the property (any data type)

	Example usage:
	   vngen_set_prop("John Doe", vngen_type_char, prop._anim, anim_wiggle);
	*/
	...
	//Get target property
	var ds_prop = argument[argument_count - 2];
	var ds_val = argument[argument_count - 1];
	      ...
	      if (global.ds_ptype == vngen_type_attach) {
	         if (ds_character[# prop._id, global.ds_pyindex] == name) or (name == -1) {
	            if (global.ds_pdata[# prop._id, global.ds_pxindex] == argument[0]) {
	               //Set target value
		           global.ds_pdata[# ds_prop, global.ds_pxindex] = ds_val;
			       exit;
	            }
	         }
	      } else {
	         ...
	         if (global.ds_pdata[# prop._id, global.ds_pyindex] == argument[0]) {
	            //Set target value
	            global.ds_pdata[# ds_prop, global.ds_pyindex] = ds_val;
	            exit;
	         }
	      }
	   }
	            ...
	            if (ds_exists(ds_data, ds_type_grid)) {
	               for (ds_xindex = 0; ds_xindex < ds_grid_height(ds_data); ds_xindex += 1) {
	                  ...
	                  if (ds_data[# prop._id, ds_xindex] == argument[0]) {
						 ...
						 //Set target value
	                     ds_data[# ds_prop, ds_xindex] = ds_val;
			             exit;
	                  }               
	               }
	            }
	      ...
	      if (ds_exists(global.ds_log_button, ds_type_grid)) {
	         for (ds_yindex = 0; ds_yindex < ds_grid_height(global.ds_log_button); ds_yindex += 1) {
	            ...
	            if (global.ds_log_button[# prop._id, ds_yindex] == argument[0]) {
				   ...
				   //Set target value
	               global.ds_log_button[# ds_prop, ds_yindex] = ds_val;
				   exit;
	            }
	         }
	      }
	      ...
	      if (ds_exists(ds_button, ds_type_grid)) {
	         for (ds_yindex = 0; ds_yindex < ds_grid_height(ds_button); ds_yindex += 1) {
	            ...
	            if (ds_button[# prop._id, ds_yindex] == argument[0]) {
				   ...
				   //Set target value
	               ds_button[# ds_prop, ds_yindex] = ds_val;
				   exit;
	            }
	         }
	      }
	   ...
	   if (ds_exists(ds_data, ds_type_grid)) {
	      ...
	      for (ds_yindex = 0; ds_yindex < ds_grid_height(ds_data); ds_yindex += 1) {
	         ...
	         if (ds_data[# prop._id, ds_yindex] == argument[0]) {
				...
				//Set target value
	            ds_data[# ds_prop, ds_yindex] = ds_val;
				exit;
			 }
	      }
	   }

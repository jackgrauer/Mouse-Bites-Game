/// @function	sys_layer_draw_button_legacy();
...
function sys_layer_draw_button_legacy() {
	...
	Draws buttons with reduced rendering features for performance and compatibility 
	...
	for (ds_yindex = 0; ds_yindex < ds_grid_height(ds_button); ds_yindex += 1) {
	   ...
	   if (ds_button[# prop._id, ds_yindex] != -1) {
	      ...
	      if (global.vg_log_visible == false) and (global.vg_ui_visible == true) and (option_hover == -1) {
	         ...
	         //Unhover button if option is hovered over same area
	         button_hover = -1;
	         button_active = -1;
	      }

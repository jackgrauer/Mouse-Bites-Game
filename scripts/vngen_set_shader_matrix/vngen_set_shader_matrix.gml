/// @function	vngen_set_shader_matrix(id, type, [name], uniform);
...
function vngen_set_shader_matrix() {

	/*
	Assigns the current transform matrix to be passed into a shader, if any is 
	active on the given entity.
	...
	Example usage:
	   vngen_set_shader_matrix("bg", vngen_type_scene, "matrix0");
	*/
	...
	ds_uniform = ds_data[# prop._sh_mat_data, ds_target];

	//Create uniform data structure, if none exists
	if (!ds_exists(ds_uniform, ds_type_grid)) {
	   ds_data[# prop._sh_mat_data, ds_target] = ds_grid_create(2, 0);
	   ds_uniform = ds_data[# prop._sh_mat_data, ds_target];
	}

/// @function	sys_layer_draw_log(spr_background, spr_divider, spr_paused, spr_playing, sep, linebreak, font, color);
...
function sys_layer_draw_log(argument0, argument1, argument2, argument3, argument4, argument5, argument6, argument7) {
	...
	Example usage:
	   sys_layer_draw_log(spr_backlog, spr_divider, spr_audio_off, spr_audio_on, 1200, fnt_Arial, c_white);
	*/
	...
	*/
	...
	for (ds_yindex = ds_grid_height(global.ds_log) - 1; ds_yindex >= 0; ds_yindex -= 1) {  
	   ...
	   for (ds_xindex = ds_grid_height(ds_log_text) - 1; ds_xindex >= 0; ds_xindex -= 1) {
	      ...
	      //Generate text surfaces when on-screen
	      if (surface_exists(text_surf)) {
	         //Reset drawing alpha for generating surface
	         draw_set_alpha(1);
	      ...
	      //Draw text surface
	      draw_set_alpha(ds_log_text[# prop._alpha, ds_xindex]);
	      draw_surface(ds_log_text[# prop._surf, ds_xindex], (gui_width - argument5)*0.5, log_yorig - ds_log_text[# prop._height, ds_xindex]);
	      draw_set_alpha(global.vg_log_alpha);

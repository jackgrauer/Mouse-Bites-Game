/// @function	vngen_label_shader_start(id, shader, fade, [ease]);
...
function vngen_label_shader_start() {

	/*
	Applies a shader to the specified label with optional fade in transition. 
	...
	argument0 = identifier of the label on which to perform shader (real or string) (or keyword 'all' for all)
	...
	Example usage:
	   vngen_event() {
	      vngen_label_shader_start("label", shade_invert, 0.5);
	   }
	*/
	...
	*/
	...
	switch (sys_action_init()) {
	   ...
	   case true: {
	      //Get label slot
	      if (argument[0] == all) {
	         //Animate all labels, if enabled
	         var ds_target = sys_grid_last(ds_label);
	         var ds_yindex = 0;
	      } else {
	         //Otherwise get single label
	      ...
	      if (!is_undefined(ds_target)) { 
	         ...
	         if (sys_shader_exists(argument[1])) {
	            while (ds_target >= ds_yindex) {  
	               //Initialize shader
	               sys_shader_init(ds_label, ds_target, argument[1]);
            
	               //Set transition time
	               if (argument[2] > 0) {
	                  ds_label[# prop._sh_time, ds_target] = 0;  //Transition time
	               } else {
	                  ds_label[# prop._sh_time, ds_target] = -1; //Transition time
	               }   
	...
	*/
	...
	//Skip action if shader does not exist
	if (!sys_shader_exists(ds_label[# prop._shader, ds_target])) {
	   exit;
	}
	...
	*/
	...
	while (ds_target >= ds_yindex) { 
	   ...
	   if (ds_label[# prop._sh_time, ds_target] < argument[2]) {
	      if (!sys_action_skip()) {
	         ds_label[# prop._sh_time, ds_target] += time_frame;
	      } else {
	         //Otherwise skip transition if vngen_do_continue is run
	         ds_label[# prop._sh_time, ds_target] = argument[2];
	      }
      
	      //Mark this action as complete
	      if (ds_label[# prop._sh_time, ds_target] < 0) or (ds_label[# prop._sh_time, ds_target] >= argument[2]) { 
	         //Disallow exceeding target values    
	         ds_label[# prop._sh_time, ds_target] = argument[2]; //Time
	         ds_label[# prop._sh_amt, ds_target] = 1;            //Amount, or fade percentage
	   ...
	   if (argument[2] > 0) {
	      ...
	      //Get transition time
	      var action_time = interp(0, 1, ds_label[# prop._sh_time, ds_target]/argument[2], action_ease);
   
	      //Perform shader fade
	      ds_label[# prop._sh_amt, ds_target] = lerp(0, 1, action_time); //Amount, or fade percentage
	   }

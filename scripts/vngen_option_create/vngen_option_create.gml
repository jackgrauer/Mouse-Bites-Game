/// @function	vngen_option_create(id, text, sprite, spr_hover, spr_select, x, y, z, font, color, transition, duration, [ease]);
	...
	Example usage:
	   if vngen_event() {
	      if vngen_option("options", 0, 0, 0.5, 0.5, snd_hover, snd_select) {
	         vngen_option_create("op1", "Option 1", spr_option, spr_option_hover, spr_option_select, -256, 0, -1, fnt_Arial, c_white, trans_fade, 1);
	         vngen_option_create("op2", "Option 2", spr_option, spr_option_hover, spr_option_select, -256, 50, -2, fnt_Arial, c_white, trans_fade, 1, true);
	      }
	...
	*/
	...
	switch (sys_option_init()) {
	   ...
	   case true: {
	      ...
	      ds_option[# prop._anim_tmp_col1, ds_target] = argument[9]; //Hover text color
	      ds_option[# prop._anim_tmp_x, ds_target] = argument[5];    //Hover X
	      ds_option[# prop._anim_tmp_y, ds_target] = argument[6];    //Hover Y
	      ...
	      ds_option[# prop._anim_col1, ds_target] = argument[9];     //Select text color
	      ds_option[# prop._anim_x, ds_target] = argument[5];        //Select X
	      ds_option[# prop._anim_y, ds_target] = argument[6];        //Select Y
	      ...
	      sys_trans_init(ds_option, ds_target, argument[10], argument[11], false);
	...
	if (option_result != -1) or (sys_event_skip()) {   
	   ...
	   if (option_pause <= 0) {
	      ...
	      if (ds_option[# prop._trans_rev, ds_target] == false) {
	         sys_trans_init(ds_option, ds_target, argument[10], argument[11], true);
	      }     
	         ...
	         */
	         ...
	         if (ds_option[# prop._trans_time, ds_target] >= ds_option[# prop._trans_dur, ds_target]) {
	            ...
	            if (!sys_event_skip()) {
				   ...
				   if (argument[1] != "") {
					  vngen_log_add(auto, "   " + string(ds_option[# prop._number, ds_target]) + ") " + status + argument[1], "", argument[8], argument[9], argument[9], argument[9], argument[9], none, none, fa_left, ds_option[# prop._txt_line_height, ds_target]);
				   }
	            }

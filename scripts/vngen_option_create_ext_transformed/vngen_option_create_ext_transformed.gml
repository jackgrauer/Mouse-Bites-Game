/// @function	vngen_option_create_ext_transformed(id, text, sprite, spr_hover, spr_select, xorig, yorig, x, y, z, tx, ty, scaling, font, color, hov_x, hov_y, hov_xscale, hov_yscale, hov_color, sel_x, sel_y, sel_xscale, sel_yscale, sel_color, sel_duration, trans_in, trans_out, duration, ease);
...
/// @param		{real}			hov_x
/// @param		{real}			hov_y
/// @param		{real}			hov_xscale
/// @param		{real}			hov_yscale
/// @param		{color}			hov_color
/// @param		{real}			sel_x
/// @param		{real}			sel_y
/// @param		{real}			sel_xscale
/// @param		{real}			sel_yscale
...
function vngen_option_create_ext_transformed() {
	...
	argument15 = sets the horizontal offset to display the option when hovered (real)
	argument16 = sets the vertical offset to display the option when hovered (real)
	argument17 = sets the horizontal scale multiplier to display the option when hovered (real)
	argument18 = sets the vertical scale multiplier to display the option when hovered (real)
	argument19 = sets the option text color to display when hovered (color)
	argument20 = sets the horizontal offset to display the option when selected (real)
	argument21 = sets the vertical offset to display the option when selected (real)
	argument22 = sets the horizontal scale multiplier to display the option when selected (real)
	argument23 = sets the vertical scale multiplier to display the option when selected (real)
	argument24 = sets the option text color to display when selected (color)
	argument25 = sets the length of hover/select animations, in seconds (real)
	argument26 = sets the transition in script to perform (script) (optional, use 'none' for none)        
	argument27 = sets the transition out script to perform (script) (optional, use 'none' for none)        
	argument28 = sets the length of the transition, in seconds (real)
	argument29 = sets the transition easing override (see interp for available modes) (integer) (can also use true/false)

	Example usage:
	   if vngen_event() {
	      if vngen_option("options", 0, 0, 0.5, 0.5, snd_hover, snd_select) {
	         vngen_option_create("op1", "Option 1", spr_option, spr_option_hover, spr_option_select, 0, 0, -256, 0, -1, 72, 16, 0, fnt_Arial, c_white, 32, 0, 1.25, 1.25, c_black, 0, 0, 1, 1, c_yellow, 0.1, trans_fade, trans_fade, 1, true);
	         vngen_option_create("op2", "Option 2", spr_option, spr_option_hover, spr_option_select, 0, 0, -256, 50, -2, 72, 16, 0, fnt_Arial, c_white, 32, 0, 1.25, 1.25, c_black, 0, 0, 1, 1, c_gray, 0.1, trans_fade, trans_fade, 1, true);
	      }
	...
	*/
	...
	switch (sys_option_init()) {
	   ...
	   case true: {
	      ...
	      ds_option[# prop._anim_tmp_col1, ds_target] = argument[19];            //Hover text color
	      ds_option[# prop._anim_tmp_x, ds_target] = argument[7] + argument[15]; //Hover X
	      ds_option[# prop._anim_tmp_y, ds_target] = argument[8] + argument[16]; //Hover Y
	      ds_option[# prop._anim_tmp_xscale, ds_target] = argument[17];          //Hover X scale
	      ds_option[# prop._anim_tmp_yscale, ds_target] = argument[18];          //Hover Y scale
      
	      //Set select properties
	      ds_option[# prop._anim_col1, ds_target] = argument[24];                //Select text color
	      ds_option[# prop._anim_x, ds_target] = argument[7] + argument[20];     //Select X
	      ds_option[# prop._anim_y, ds_target] = argument[8] + argument[21];     //Select Y
	      ds_option[# prop._anim_xscale, ds_target] = argument[22];              //Select X scale
	      ds_option[# prop._anim_yscale, ds_target] = argument[23];              //Select Y scale
      
	      //Set timing properties
	      ds_option[# prop._anim_time, ds_target] = 0;                           //Animation time
	      ds_option[# prop._anim_dur, ds_target] = max(0.01, argument[25]);      //Animation duration
	      ds_option[# prop._anim_ease, ds_target] = argument[29];                //Animation ease
	      ...
	      sys_trans_init(ds_option, ds_target, argument[26], argument[28], false);
	...
	*/
	...
	if (option_result == -1) {
	   if (ds_option[# prop._trans_time, ds_target] < ds_option[# prop._trans_dur, ds_target]) {
	      //Perform transitions
	      sys_trans_perform(ds_option, ds_target, argument[29]);
	   }
	} 
	...
	if (option_result != -1) or (sys_event_skip()) {     
	   ...
	   if (option_pause <= 0) {
	      ...
	      if (ds_option[# prop._trans_rev, ds_target] == false) {
	         sys_trans_init(ds_option, ds_target, argument[27], argument[28], true);   
	      }
	         ...
	         if (ds_option[# prop._trans_time, ds_target] >= ds_option[# prop._trans_dur, ds_target]) {
	            ...
	            if (!sys_event_skip()) {
	               ...
	               if (option_result == argument[0]) {
	                  var status = "(*) ";
	                  var text_col = argument[24];
	               } else {
	                  var status = "";
	                  var text_col = argument[14];
	               }
         
	               //Add option to backlog
				   if (argument[1] != "") {
	                  vngen_log_add(auto, "   " + string(ds_option[# prop._number, ds_target]) + ") " + status + argument[1], "", argument[13], text_col, text_col, text_col, text_col, none, none, fa_left, ds_option[# prop._txt_line_height, ds_target]);
				   }
	            }

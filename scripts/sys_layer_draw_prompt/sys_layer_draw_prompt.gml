/// @function	sys_layer_draw_prompt();
	...
	for (ds_yindex = 0; ds_yindex < ds_grid_height(ds_prompt); ds_yindex += 1) {
	   ...
	   if (ds_prompt[# prop._id, ds_yindex] != -1) {
	      ...
	      for (ds_xindex = 0; ds_xindex < action_trigger_count; ds_xindex += 1) {       
	         ...
	         //Get deform surfaces
	         var action_def_surf = ds_prompt[# prop._def_surf, ds_yindex];
	         var action_def_fade_surf = ds_prompt[# prop._def_fade_surf, ds_yindex];  
	         ...
	         if (ds_xindex == 0) {
	            ...
	            sys_deform_perform(ds_prompt, ds_yindex);   
      
	            /* SHADERS */
            
	            //Perform shader, if any
	            sys_shader_perform(ds_prompt, ds_yindex);   
	         } 
	         ...
	         if (action_char == true) { 
	            ...
	            if (ds_prompt[# prop._def, ds_yindex] == -1) {
	            ...
	            /* PRIMITIVE */
	               ...
	               if (sprite_exists(action_sprite)) {
	                  ...
	                  var action_surf_width = action_width;
	                  var action_surf_height = action_height;
	               }
	               ...
	               if (!surface_exists(action_def_surf)) {
	                  ds_prompt[# prop._def_surf, ds_yindex] = surface_create(action_surf_width, action_surf_height);
	                  action_def_surf = ds_prompt[# prop._def_surf, ds_yindex];
	               } else {
	                  //Ensure surface matches active sprite dimensions
	                  if (sprite_exists(action_sprite)) {
	                     if (surface_get_width(action_def_surf) != action_surf_width) or (surface_get_height(action_def_surf) != action_surf_height) {
	                        surface_resize(action_def_surf, action_surf_width, action_surf_height);
	                     }
	                  }
	               }
	               ...
	               if (sprite_exists(action_sprite)) {
	                  draw_sprite_general(action_sprite, sprite_get_index(action_sprite), action_left, action_top, action_width, action_height, action_left, action_top, 1, 1, 0, action_col1, action_col2, action_col3, action_col4, 1);
	               }
         
	               //Temporarily disable shader, if any
	               if (sys_shader_exists(ds_prompt[# prop._shader, ds_yindex])) {
	                  shader_reset();
	               }
	               ...
	               //Draw as primitive
	               if (sprite_exists(action_sprite)) {
	                  sys_deform_draw(ds_prompt, ds_yindex, surface_get_texture(action_def_surf), action_surf_width, action_surf_height, action_x, action_y, action_xscale, action_yscale, action_rot, ((action_alpha*action_anim_alpha)*action_fade_alpha)*global.vg_ui_alpha); 
	               }
         
	               //Re-enable shader, if any
	               if (sys_shader_exists(ds_prompt[# prop._shader, ds_yindex])) {
	                  shader_set(ds_prompt[# prop._shader, ds_yindex]);
	               }
	               ...
	               if (sprite_exists(action_fade_sprite)) {
	                  ...
	                  if (!surface_exists(action_def_fade_surf)) {
	                     ds_prompt[# prop._def_fade_surf, ds_yindex] = surface_create(action_surf_width, action_surf_height);
	                     action_def_fade_surf = ds_prompt[# prop._def_fade_surf, ds_yindex];
	                  }     
            
	                  //Draw to surface for deformation
	                  surface_set_target(action_def_fade_surf);
	                  draw_clear_alpha(c_black, 0);
	                  draw_sprite_general(action_fade_sprite, sprite_get_index(action_fade_sprite), 0, 0, action_surf_width, action_surf_height, 0, 0, 1, 1, 0, action_col1, action_col2, action_col3, action_col4, 1);
	                  ...
	                  //Draw fade primitive
	                  sys_deform_draw(ds_prompt, ds_yindex, surface_get_texture(action_def_fade_surf), action_surf_width, action_surf_height, action_x, action_y, action_xscale, action_yscale, action_rot, (action_anim_alpha*action_fade_alpha)*global.vg_ui_alpha);      
	               }
	            }
	         }       
	      } 

/// @function	vngen_scene_shader_start(id, shader, fade, [ease]);
...
function vngen_scene_shader_start() {

	/*
	Applies a shader to the specified scene with optional fade in transition. 
	...
	argument0 = identifier of the scene on which to perform shader (real or string) (or keyword 'all' for all)
	      ...
	      if (!is_undefined(ds_target)) { 
	         ...
	         if (sys_shader_exists(argument[1])) {
	            while (ds_target >= ds_yindex) {  
	               //Initialize shader
	               sys_shader_init(ds_scene, ds_target, argument[1]);
            
	               //Set transition time
	               if (argument[2] > 0) {
	                  ds_scene[# prop._sh_time, ds_target] = 0;  //Transition time
	               } else {
	                  ds_scene[# prop._sh_time, ds_target] = -1; //Transition time
	               }   
	...
	*/
	...
	//Skip action if shader does not exist
	if (!sys_shader_exists(ds_scene[# prop._shader, ds_target])) {
	   exit;
	}
	...
	*/
	...
	while (ds_target >= ds_yindex) { 
	   ...
	   if (ds_scene[# prop._sh_time, ds_target] < argument[2]) {
	      if (!sys_action_skip()) {
	         ds_scene[# prop._sh_time, ds_target] += time_frame;
	      } else {
	         //Otherwise skip transition if vngen_do_continue is run
	         ds_scene[# prop._sh_time, ds_target] = argument[2];
	      }
      
	      //Mark this action as complete
	      if (ds_scene[# prop._sh_time, ds_target] < 0) or (ds_scene[# prop._sh_time, ds_target] >= argument[2]) { 
	         //Disallow exceeding target values    
	         ds_scene[# prop._sh_time, ds_target] = argument[2]; //Time
	         ds_scene[# prop._sh_amt, ds_target] = 1;            //Amount, or fade percentage
	   ...
	   if (argument[2] > 0) {
	      ...
	      //Get transition time
	      var action_time = interp(0, 1, ds_scene[# prop._sh_time, ds_target]/argument[2], action_ease);
   
	      //Perform shader fade
	      ds_scene[# prop._sh_amt, ds_target] = lerp(0, 1, action_time); //Amount, or fade percentage
	   }

/// @function	vngen_perspective_replace(x, y, xoffset, yoffset, zoom, rot, strength, duration, [ease]);
/// @param		{real}			x
/// @param		{real}			y
/// @param		{real}			xoffset
/// @param		{real}			yoffset
...
function vngen_perspective_replace() {

	/*
	Fades to a new global offset to the position, rotation, and scale of active scenes, 
	...
	argument7 = sets the length of the perspective fade transition, in seconds (real)
	argument8 = sets the animation easing method (see interp for available modes) (integer) (can also use true/false)

	Example usage:
	   vngen_event() {
	      vngen_perspective_replace(0, 0, -50, 50, 1, 0, 1, 0.5, true);
	   }
	*/
	...
	*/
	...
	switch (sys_action_init()) {
	   ...
	   case true: {
	      ...
	      //Create fade surface, if none exists
	      if (!surface_exists(ds_perspective[# prop._fade_src, 0])) { //Fade surface 
	         ds_perspective[# prop._fade_src, 0] = surface_create(surface_get_width(application_surface), surface_get_height(application_surface));
	      }
   
	      //Enable fade surface visibility
	      ds_perspective[# prop._fade_alpha, 0] = 1;
   
	      //Enable capturing current perspective
	      ds_perspective[# prop._fade_draw, 0] = true;
	      exit;
	   }
	}
	...
	*/
	...
	//Set new perspective once old perspective has been captured
	if (ds_perspective[# prop._fade_draw, 0] == true) {      
	   ...
	   ds_perspective[# prop._y, 0] = argument[1];        //Y ycenter   
	   ...
	   ds_perspective[# prop._fade_draw, 0] = false;      //Perspective capture
	}
	...
	*/
	...
	if (ds_perspective[# prop._time, 0] < argument[7]) {
	   ...
	   if (ds_perspective[# prop._time, 0] < 0) or (ds_perspective[# prop._time, 0] >= argument[7]) { 
	      ...
	      ds_perspective[# prop._fade_alpha, 0] = 0;     //Alpha
	      ds_perspective[# prop._fade_draw, 0] = false;  //Fade draw
	      ds_perspective[# prop._time, 0] = argument[7]; //Time
      
	      //Remove fade data from memory
	      if (surface_exists(ds_perspective[# prop._fade_src, 0])) {
	         surface_free(ds_perspective[# prop._fade_src, 0]);
	      }
	      ds_perspective[# prop._fade_src, 0] = -1;
	...
	if (argument[7] > 0) {
	   ...
	   ds_perspective[# prop._fade_alpha, 0] = lerp(1, 0, action_time); //Alpha
	}

/// @function	sys_layer_draw_text(highlight);
/// @param		{boolean} highlight
/// @author		Lucas Chasteen <lucas.chasteen@xgasoft.com>
/// @copyright	XGASOFT 2020, All Rights Reserved
function sys_layer_draw_text(argument0) {
	...
	argument0 = enables or disables highlighting text links when hovered (boolean) (true/false)
            
	Example usage:
	   sys_layer_draw_text(true);
	*/
	...
	*/
	...
	for (ds_yindex = 0; ds_yindex < ds_grid_height(ds_text); ds_yindex += 1) {
	   ...
	   if (ds_text[# prop._id, ds_yindex] != -1) {
	      ...
	      //Get deform surfaces
	      var action_def_surf = ds_text[# prop._def_surf, ds_yindex];
	      var action_def_fade_surf = ds_text[# prop._def_fade_surf, ds_yindex];  
	      ...
	      sys_deform_perform(ds_text, ds_yindex);
      
	      /* SHADERS */
      
	      //Perform shader, if any
	      sys_shader_perform(ds_text, ds_yindex);  
	      ...
	      /* PRIMITIVE */
	         ...
	         if (!surface_exists(action_def_surf)) {
	            ds_text[# prop._def_surf, ds_yindex] = surface_create(action_surf_width, action_surf_height);
	            action_def_surf = ds_text[# prop._def_surf, ds_yindex];
	         }
	         ...
	         if (sys_shader_exists(ds_text[# prop._shader, ds_yindex])) {
	            shader_reset();
	         }
	         ...
	         //Draw as primitive
	         sys_deform_draw(ds_text, ds_yindex, surface_get_texture(action_def_surf), action_surf_width, action_surf_height, action_x, action_y, action_xscale, action_yscale, action_rot, ((action_alpha*action_anim_alpha)*action_fade_alpha)*global.vg_ui_alpha);
         
	         //Re-enable shader, if any
	         if (sys_shader_exists(ds_text[# prop._shader, ds_yindex])) {
	            shader_set(ds_text[# prop._shader, ds_yindex]);
	         }
	         ...
	         if (surface_exists(action_fade_surf)) {
	            ...
	            if (!surface_exists(action_def_fade_surf)) {
	               ds_text[# prop._def_fade_surf, ds_yindex] = surface_create(action_surf_width, action_surf_height);
	               action_def_fade_surf = ds_text[# prop._def_fade_surf, ds_yindex];
	            }     
	            ...
	            //Draw fade primitive
	            sys_deform_draw(ds_text, ds_yindex, surface_get_texture(action_def_fade_surf), action_surf_width, action_surf_height, action_x, action_y, action_xscale, action_yscale, action_rot, (action_anim_alpha*action_fade_alpha)*global.vg_ui_alpha);      
	         }
	      } 
	                  ...
	                  if (global.vg_pause == false) {
	                     ...
	                     if (mouse_check_region(link_x, link_y, link_width, link_height)) {
	                        ...
	                        if (argument0 == true) {
	                           ...
	                           draw_surface_part_ext(action_surf, ds_link[# prop._x, ds_xindex], ds_link[# prop._y, ds_xindex], ds_link[# prop._width, ds_xindex], ds_link[# prop._height, ds_xindex], link_x, link_y, action_xscale, action_yscale, c_gray, 1);

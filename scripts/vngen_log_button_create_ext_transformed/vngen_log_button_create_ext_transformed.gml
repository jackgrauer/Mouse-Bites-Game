/// @function	vngen_log_button_create_ext_transformed(id, text, sprite, spr_hover, spr_select, xorig, yorig, x, y, z, tx, ty, scaling, font, color, hov_x, hov_y, hov_xscale, hov_yscale, hov_color, sel_x, sel_y, sel_xscale, sel_yscale, sel_color, sel_duration, snd_hover, snd_select, ease);
...
function vngen_log_button_create_ext_transformed() {
	...
	argument28 = sets the transition easing override (see interp for available modes) (integer) (can also use true/false)

	Example usage:
	   vngen_log_button_create_ext_transformed("btn", "Click me!", spr_btn, spr_btn_hover, spr_btn_select, 0, 0, 256, 512, 0, 72, 16, 0, fnt_Arial, c_white, 32, 0, 1.25, 1.25, c_black, 0, 0, 1, 1, c_yellow, 0.1, snd_hover, snd_select, true);
	*/
	...
	*/
	...
	global.ds_log_button[# prop._snd_hover, ds_target] = argument[26];						 //Hover sound
	global.ds_log_button[# prop._snd_select, ds_target] = argument[27];						 //Select sound
	...
	global.ds_log_button[# prop._anim_tmp_col1, ds_target] = argument[19];            //Hover text color
	global.ds_log_button[# prop._anim_tmp_x, ds_target] = argument[7] + argument[15]; //Hover X
	global.ds_log_button[# prop._anim_tmp_y, ds_target] = argument[8] + argument[16]; //Hover Y
	global.ds_log_button[# prop._anim_tmp_xscale, ds_target] = argument[17];          //Hover X scale
	global.ds_log_button[# prop._anim_tmp_yscale, ds_target] = argument[18];          //Hover Y scale

	//Set select properties
	global.ds_log_button[# prop._anim_col1, ds_target] = argument[24];                //Select text color
	global.ds_log_button[# prop._anim_x, ds_target] = argument[7] + argument[20];     //Select X
	global.ds_log_button[# prop._anim_y, ds_target] = argument[8] + argument[21];     //Select Y
	global.ds_log_button[# prop._anim_xscale, ds_target] = argument[22];              //Select X scale
	global.ds_log_button[# prop._anim_yscale, ds_target] = argument[23];              //Select Y scale

	//Set timing properties
	global.ds_log_button[# prop._anim_time, ds_target] = 0;                           //Animation time
	global.ds_log_button[# prop._anim_dur, ds_target] = max(0.01, argument[25]);      //Animation duration
	global.ds_log_button[# prop._anim_ease, ds_target] = argument[28];                //Animation ease

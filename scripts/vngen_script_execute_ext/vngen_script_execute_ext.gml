/// @function	vngen_script_execute_ext(script, [argument0], [argument1] ... [argument31]);
/// @param		{script}	script
...
function vngen_script_execute_ext() {

	/*
	Executes arbitrary script code as a VNgen action. While it may seem identical to the standard
	vngen_script_execute, vngen_script_execute_ext executes scripts with one critical difference:
	script code will also be executed during vngen_goto operations to reconstruct the target event.
	To execute code which should not be repeated, use vngen_script_execute instead.

	Note that executed code is limited to the object event where VNgen actions are run (usually Step).

	Example usage:
	   vngen_event() {
	      vngen_script_execute_ext(my_script, 1, 2, 3, true, false);
	   }
	*/

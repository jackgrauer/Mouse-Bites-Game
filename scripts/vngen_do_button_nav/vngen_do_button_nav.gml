/// @function	vngen_do_button_nav(amount);
/// @param		{integer}	amount
/// @author		Lucas Chasteen <lucas.chasteen@xgasoft.com>
/// @copyright	XGASOFT 2020, All Rights Reserved
function vngen_do_button_nav(argument0) {

	/*
	Navigates through a linear list of buttons by the amount specified, if buttons
	currently exist. 'Amount' refers to the number of buttons, where negative 
	...
	Note that buttos are not restricted to a certain display order, and therefore 
	...
	*/
	...
	if (button_active == -1) {
	   ...
	   //Get button ID
	   button_hover = ds_button[# prop._id, button_index];
	   ...
	   if (is_undefined(button_hover)) {
	      ...
	      ds_button[# prop._img_index, button_index] = 0;
	   }

	   //Play select sound, if any
	   if (audio_exists(ds_button[# prop._snd_hover, button_index])) {
	      var snd = audio_play_sound(ds_button[# prop._snd_hover, button_index], 0, false);

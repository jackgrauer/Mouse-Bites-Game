/// @function	vngen_do_log_button_select([id]);
/// @param		{real|string}	[id]
/// @author		Lucas Chasteen <lucas.chasteen@xgasoft.com>
/// @copyright	XGASOFT 2020, All Rights Reserved
function vngen_do_log_button_select() {
	...
	Example usage:
	   ...
	   if (keyboard_check_released(ord("L"))) {
	      vngen_do_log_button_select("btn_close");
	   }
	*/
	...
	if (button_active == -1) {
	   ...
	   if (button_hover != -1) {
		  ...
		  if (!is_undefined(ds_target)) {
	         if (audio_exists(global.ds_log_button[# prop._snd_select, ds_target])) {
	            var snd = audio_play_sound(global.ds_log_button[# prop._snd_select, ds_target], 0, false);

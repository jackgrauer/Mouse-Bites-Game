/// @function	vngen_textbox_modify_pos(id, x, y, z, scale, rot, duration, [ease]);
...
function vngen_textbox_modify_pos() {
	...
	argument4 = the new scale multiplier to display the textbox (real)
	argument5 = the new rotation value to apply to the textbox, in degrees (real)
	...
	Example usage:
	   vngen_event() {
	      vngen_textbox_modify_pos("textbox", 50, 100, -1, 1.5, 45, 2);
	   }
	*/
	   ...
	   case true: {
	      ...
	      if (!is_undefined(ds_target)) {     
	         while (ds_target >= ds_yindex) {           
	            ...
	            ds_textbox[# prop._z, ds_target] = argument[3]; //Z
	            if (argument[0] != all) {
	...
	while (ds_target >= ds_yindex) { 
	   ...
	   if (ds_textbox[# prop._time, ds_target] < argument[6]) {
	      ...
	      if (ds_textbox[# prop._time, ds_target] < 0) or (ds_textbox[# prop._time, ds_target] >= argument[6]) {      
	         ...
	         ds_textbox[# prop._x, ds_target] = argument[1];        //X
	         ds_textbox[# prop._y, ds_target] = argument[2];        //Y
	         ds_textbox[# prop._xscale, ds_target] = argument[4];   //X scale
	         ds_textbox[# prop._yscale, ds_target] = argument[4];   //Y scale
	         ds_textbox[# prop._rot, ds_target] = argument[5];      //Rotation
	   ...
	   if (argument[6] > 0) {
	      ...
	      ds_textbox[# prop._x, ds_target] = lerp(ds_textbox[# prop._tmp_x, ds_target], argument[1], action_time);           //X
	      ds_textbox[# prop._y, ds_target] = lerp(ds_textbox[# prop._tmp_y, ds_target], argument[2], action_time);           //Y
	      ds_textbox[# prop._xscale, ds_target] = lerp(ds_textbox[# prop._tmp_xscale, ds_target], argument[4], action_time); //X scale
	      ds_textbox[# prop._yscale, ds_target] = lerp(ds_textbox[# prop._tmp_yscale, ds_target], argument[4], action_time); //Y scale
	      ds_textbox[# prop._rot, ds_target] = lerp(ds_textbox[# prop._tmp_rot, ds_target], argument[5], action_time);       //Rotation
	   }
